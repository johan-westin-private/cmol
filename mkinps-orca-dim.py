...
ROUTE = (
    '!b97-d3 def2-tzvp def2-tzvp/j'
    ' ecp(def2-tzvp,def2-tzvp/j) slowconv nososcf'
)
...
if __name__ == '__main__':
    ...
    description = (
        'Script for construction of ORCA inputs from XYZ files.'
        ' Optionally creates input files for BSSE correction.'
        ' Reads <name>-d???.xyz and <name>-m??.xyz files.'
    )
    ...
    if m_options.bsse:
        ...
        for m_xyzfile in m_xyz_d:
            ...
            assert len(m_mols) == 2, \
                "Expected 2 molecules in file %s, found %i" % (
                    m_xyzfile, len(m_mols)
                )
